...
async function validateOpenAIKey() {
    ...
    try {
        // Teste API Key mit einfacher Anfrage
...
async function transcribeAudio(audioFilePath) {
    ...
    try {
        ...
        // Prüfe Dateigröße (Whisper Limit: 25MB)
        ...
        if (!response.ok) {
            ...
            try {
                ...
                throw new Error(`Whisper API Fehler: ${response.status} ${response.statusText} - ${errorText}`);
            }
    ...
    try {
        ...
        if (!process.env.OPENAI_API_KEY) {
            return res.status(400).json({ 
                ...
                solution: 'Setzen Sie OPENAI_API_KEY in der .env Datei'
            });
        }

        // Test mit einfacher Anfrage
        const testMessage = "Antworte nur mit 'OK' wenn du mich verstehst.";
        
        const result = await callOpenAI([
            { role: "system", content: "Du bist ein Test-Assistent. Antworte kurz und präzise." },
        ...
        res.status(500).json({
            ...
            details: {
                ...
                suggestion: error.message.includes('API Key') 
                    ? 'Prüfen Sie Ihren OpenAI API Key'
...
Wichtig: Du ersetzt keine professionelle Supervision oder Ausbildung, sondern ergänzt diese.`;
        ...
        if (client_id) {
          const history = getChatHistory(client_id, 3); // Reduziere auf 3 für bessere Performance
          history.reverse().forEach(msg => {
            if (msg.role === 'user' || msg.role === 'assistant') {
              messages.splice(-1, 0, { role: msg.role, content: msg.content.substring(0, 500) }); // Kürze lange Nachrichten
            }
          });
        }
...
// Validiere OpenAI Key beim Start
setTimeout(async () => {
