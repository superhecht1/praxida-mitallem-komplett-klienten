// db.js - Praxida 2.0 SQLite Database Layer (REPARIERT)
...
function initializeTables() {
  ...
  // Assessments Tabelle (für Outcome-Tracking)
  db.prepare(`
...
// --- ASSESSMENTS FUNKTIONEN --- //
function addAssessment(assessmentData) {
  try {
    const stmt = db.prepare(`
      INSERT INTO assessments (client_id, session_id, assessment_type, responses, total_score, severity_level, completed_at, notes)
      VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?)
    `);
    ...
    console.log("📊 Assessment hinzugefügt:", assessmentData.assessment_type, "für Client:", assessmentData.client_id);
    return result;
  } catch (error) {
  ...
  try {
    ...
    if (assessmentType) {
      stmt = db.prepare(`
        ...
        WHERE client_id = ? AND assessment_type = ?
