...
function createDemoPraxis() {
    ...
    const result = praxisStmt.run(
        'Demo Praxis Köln',
        'info@demo-praxis-koeln.de',
        '+49 221 123456',
        'Musterstraße 123\n50667 Köln',
        'https://demo-praxis-koeln.de'
    );
    
    return result.lastInsertRowid;
}

function createDemoUsers(praxisId) {
    const userStmt = db.prepare(`
        INSERT INTO users (praxis_id, name, email, password_hash, role, created_at)
        VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    `);
    
    // Demo password hash for "demo123456"
    const demoPasswordHash = '$2a$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LeZHfAMAgN9ksHO9i';
    
    const users = [
        ['Dr. Demo Admin', 'admin@demo-praxis.de', demoPasswordHash, 'admin'],
        ['Dr. Sarah Therapeutin', 'therapeut@demo-praxis.de', demoPasswordHash, 'therapeut'],
        ['Lisa Assistenz', 'assistenz@demo-praxis.de', demoPasswordHash, 'assistenz']
    ];
    
    users.forEach(user => {
        userStmt.run(praxisId, ...user);
    });
}
